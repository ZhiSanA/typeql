...
# 之后
{ users { rows { id, name }, pagination { limit, offset, count } } }
```

### 新增

- **`Pagination` 类型**：包含 `limit`、`offset`（实际请求的值）、`count`（总记录数）
- **`{TypeName}ListResult` 类型**：每个实体自动生成，包含 `rows: [Type]!` 和 `pagination: Pagination!`
...
- `resolvers.ts`：`makeList` 返回 `{ rows, pagination }` 替代原来的直接返回数组
